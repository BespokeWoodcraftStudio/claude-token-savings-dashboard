 ...
 *
 *   1. Cloud Mem  — the claude-mem SQLite DB (default ~/.claude-mem/claude-mem.db).
...
function main() {
  ...
  try { cloudMem = collectCloudMem(cfg); } catch (e) { errors.push("Cloud Mem collection failed: " + e.message); cloudMem = { notDetected: true, reason: e.message }; }
  ...
  const memMsg = memOk ? `$${combined.savedDollars.toFixed(0)} saved · ${cloudMem.observations} obs` : "Cloud Mem not detected";
